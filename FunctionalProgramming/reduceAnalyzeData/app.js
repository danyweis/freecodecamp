...
// If this second parameter is not used, then the first iteration is skipped and the second iteration gets 
...
var watchList = [
  ...
  function getRating(watchList){
    ...
    // USE FILTER TO GET AN ARRAY WITH ONLY Christopher Nolans INFOS , THEN USE MAPWITH Number() TO GET THE RATINGS ONLY

   const director = watchList.filter(name => name['Director'] === 'Christopher Nolan').map(total => Number(total.imdbRating));
   
   // WE GET THE LENGTH OF THE NEW ARRAY

   let index = director.length
  
    // WE USE REDUCE TO ADD ALL THE RATINGS TOGETHER AND DEVIED THE RESULT BY THE NUMBER OF ENTERYS
  averageRating= director.reduce((acc, cur) => acc + cur) / index;
